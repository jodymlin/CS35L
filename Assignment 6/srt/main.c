...
colorSphere( void* n )
{
    ...
    for( int px = thread_num; px < width; px += nthreads )
    {
	    ...
	    scaled_color[0] = gamma( pixel_color[0] ) * max_color;
            ...
            for( int i=0; i<3; i++)
                colorArr[px][py][i] = max( min(scaled_color[i], 255), 0);
        }
    }
...
main( int argc, char **argv )
{
    ...
    */
    ...
    int rc;                    // return value
    int data[nthreads];        // data passed to thread

    int t;
    for (t = 0; t < nthreads; t++) {
      data[t] = t;
      rc = pthread_create(&tid[t], NULL, colorSphere, &data[t]);
      if (rc) {
    ...
    /* write pixels out to disk. ppm is forgiving about whitespace,
    ...
    for (i = 0; i < width; i++)
    {
      ...
      {
	printf( "%.0f %.0f %.0f\n",
		colorArr[i][j][0], colorArr[i][j][1], colorArr[i][j][2] );
      }
      printf( "\n" );
    }
